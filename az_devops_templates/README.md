...
## 📖 Overview
These templates are organized into three main categories: Common Steps, Standardized Jobs, and Pipeline Examples.

## 📜 Templates Overview

### 1. Common Steps (`common/`)
- **security_scans.yml**: Integrated Gitleaks, SonarCloud, Trivy, and JFrog Xray.
...
- **tag_watcher_task.yml**: Checks for new tags and triggers a pipeline.
- **notifications.yml**: Reusable steps for Slack notifications.
- **run_script_util.yml**: Helper for running external shell scripts.

### 2. Standardized Jobs (`jobs/`)
- **build_test_job.yml**: Multi-language job supporting .NET, C++, Node.js, and Python.
...
- **utils/tag_watcher_pipeline.yml**: Scheduled pipeline to monitor tags.
...
- **Container Scanning**: Trivy scans images before they are pushed.
