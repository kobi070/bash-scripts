...
while [ "$SECONDS" -lt "$DURATION" ]; do
    ...
    # Update stats securely by passing variables to awk via -v
...
if [ "$COUNT" -gt 0 ]; then
    AVG_CPU=$(awk -v sum_cpu="$SUM_CPU" -v count="$COUNT" "BEGIN {printf \"%.2f\", sum_cpu / count}")
    AVG_MEM=$(awk -v sum_mem="$SUM_MEM" -v count="$COUNT" "BEGIN {printf \"%.2f\", sum_mem / count}")
