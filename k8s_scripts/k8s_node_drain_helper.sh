...
# This reduces process forks from O(N) to O(1) by performing PDB correlation and
# local storage detection within a single pass over the pod list.
echo "$PODS_JSON" | jq -r --argjson pdbs "$PDBS_JSON" '
  ...
  # Find matching PDBs in the same namespace using label selector matching
  ([
    $pdbs.items[] |
    select(.metadata.namespace == $ns) |
    (.spec.selector.matchLabels // {}) as $s |
    # A PDB matches if its selector is a non-empty subset of the pod labels
    ($s != {} and ($s | to_entries | all($labels[.key] == .value)))
  ] | length) as $pdb_count |
  (if $pdb_count > 0 then "OK (\($pdb_count))" else "MISSING" end) as $pdb_status |

  # Check for local storage (emptyDir volumes)
