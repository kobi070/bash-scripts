...
# Bolt optimization: Consolidate all Ingress auditing into a single jq pipeline.
# This reduces process forks from O(N) to O(1) by eliminating jq calls, paste, and sort inside the loop.
kubectl get ingress $NS_ARG -o json | jq -r '
  .items[] |
  .metadata.namespace as $ns |
  .metadata.name as $name |
  ([.spec.rules[]?.host] | if length == 0 then ["None"] else map(. // "None") end | join(",")) as $hosts |
  (if (.spec.tls | length) > 0 then "YES" else "NO" end) as $tls |
  ([.spec.rules[]?.http.paths[]?.backend.service.name // .spec.backend.service.name] | if length == 0 then ["N/A"] else map(. // "N/A") | unique end | join(",")) as $backends |
